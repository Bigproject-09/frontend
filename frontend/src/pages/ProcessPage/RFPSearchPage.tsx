...
const RFPSearchPage: React.FC = () => {
  ...
  const handleSubmit = async (id: number) => {
    ...
    try {
      ...
      // 저장된 링크도 같이 조회해서 화면에 표시
      const stored = await http.get(`/api/notices/${id}/analysis-results`);
      const refs = (stored.data.references ?? []) as any[];
      const links = refs
